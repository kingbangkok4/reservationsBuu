...
header('Content-Disposition: attachment; filename="รายงานการสั่งจองสินค้าประจำวัน.doc"');

include("config.php");
mysql_connect($dbhost, $dbuser, $dbpass) or die(mysql_error());
mysql_query("SET NAMES UTF8 ") or die (mysql_error());
$sql = "SELECT orders_detail.* , product.* FROM orders, orders_detail, product WHERE orders.Order_Id = orders_detail.Order_Id and orders_detail.Product_Code = product.Product_Code and orders_detail.type = 'Reserve' and DATE( Order_Date )=DATE(CURRENT_TIMESTAMP)"; //ดึงข้อมูลจาก เทเบิล word 
